 ...
 * @LastEditTime: 2025-02-20 15:58:21
 */
"use client";
      ...
      {tokens.length === 0 ? (
        <>
          <div className="bg-gray-800 text-gray-400 p-3 my-2 mb-4 rounded md:text-base text-xs">
            <p>
              当前钱包余额: <span className="text-green-300">{balance} SOL</span>
            </p>
          </div>
          ...
          <ul>
            {tokens.map((token, index) => (
              <li key={index} className="bg-gray-800 text-gray-400 p-3 my-1 rounded mb-4 md:text-base text-xs">
                ...
                <p>
                  小数位数: <span className="text-white">{token.decimals}</span>
                </p>
                <p>
                  铸币数量: <span className="text-white">{token.amount}</span>
                </p>
                <p>
                  关联账户地址: <span className="text-white">{token.ataAddress}</span>
                </p>
              </li>
            ))}
          </ul>
